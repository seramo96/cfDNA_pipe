...
#wig format: Chr Coord Value
	...
	if($_[0]=~/gz$/){
		open(YWIG,"gunzip -c $_[0] |") || die "cannot open pipe to $_[0]";
	}else{
	...
	while(my $bed_line=<BEDGRAPH>){
		...
		if($bed_line=~/track/){
			...
			for(my $i=$st;$i<=$en;$i+=10){
				if(exists $genome_size{$temp[0]} &&  $i<= int($genome_size{$temp[0]}) && $i>0){
...
sub readbed_ends{
	...
	while(my $bed_line=<BED>){
		...
		if($#temp < 2){
			...
			my $mp=int( ( ($temp[1]+$temp[2]) /2) + 0.5 );
			if($temp[1] <= int($genome_size{$temp[0]})){
					$twig{$chrid}{$temp[1]}++;
					$ct++;
			}if($temp[2] <= int($genome_size{$temp[0]})){
					$twig{$chrid}{$temp[2]}++;
					$ct++;
			}
		}
...
sub left_right_readbed_ends{
	my ($bed_file,$min,$max,$step,$genome,$tgs)=@_;
	my @temp; my %lwig; my %rwig; my $bed_line; my $chrid; my %tlwig; my %trwig; my $ct=0;
	my $i; my $j;
	my %genome_size = %{$tgs};
	if($bed_file=~/gz$/){
		open(BED,"gunzip -c $bed_file |") || die "cannot open pipe to $bed_file";
	...
	#normalize
	foreach $i (keys(%tlwig)){
		foreach $j ( keys(%{$tlwig{$i}}) ) {
			my $normval = $tlwig{$i}{$j}*$GN/$ct;
			$lwig{$i}{$j}=$normval;
		}
	}
	foreach $i (keys(%trwig)){
		foreach $j ( keys(%{$trwig{$i}}) ) {
			my $normval = $trwig{$i}{$j}*$GN/$ct;
			$rwig{$i}{$j}=$normval;
		}
	}
	return (\%lwig,\%rwig);
...
#wig format: Coord Value ; chr in the header only
	...
	while(my $wig_line=<WIG>){
		...
		if($wig_line=~/chrom/){
			...
			$wig{$chrid}{$temp[0]}=int($temp[1]*10000)/10000; # $Ywig{Chromosome}{coordinate} = value
		}
	}
	close(WIG);
	return (\%wig);
